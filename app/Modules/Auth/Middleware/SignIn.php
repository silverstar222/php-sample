...
class SignIn
{
    ...
    {
        try {
            $this->requestAllData = $request->only('email','password');
            $this->checkInputData($request->only('email','password','device_token'));
        ...
        $validator = Validator::make($data, [
            ...
            'password' => ['required','string','min:30',
                Rule::exists('users')->where(function ($query) {
                    $query->where('email', $this->requestAllData['email']);
                    $query->where('password', $this->requestAllData['password']);
                }),],
